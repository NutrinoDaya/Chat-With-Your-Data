...
def extract_limit_number(message: str) -> int | None:
    """Extract LIMIT number from queries like 'top 3', 'first 5', 'limit 10'."""
    m = message.lower()
    import re
    patterns = [
        r'\btop\s+(\d+)\b',
    ...
    if source == 'financial':
        ...
        if 'how many' in m or 'number of orders' in m or 'orders did we' in m or ('count' in m and 'order' in m):
            ...
            if group_by_customer:
                return f"SELECT customer, COUNT(*) AS order_count FROM {tbl} WHERE {time_filter} GROUP BY customer ORDER BY order_count DESC{limit_clause};"
            return f"SELECT COUNT(*) AS order_count FROM {tbl} WHERE {time_filter};"
        
        # Revenue with time filter (with COALESCE fix)
...
async def ask(req: ChatRequest):
    try:
        ...
        # Check for simple greetings/thanks first
        ...
        if any(pattern in message_lower for pattern in greeting_patterns):
            if "thank" in message_lower:
                response_text = "You're welcome! Feel free to ask any questions about your data."
            elif any(word in message_lower for word in ["bye", "goodbye"]):
                response_text = "Goodbye! Have a great day!"
            else:
                response_text = "Hello! I'm here to help you analyze your data. What would you like to know?"
        ...
        if mode_hint == "SQL":
            ...
            if rule_sql:
                ...
                # Post-process LLM SQL to add missing LIMIT clauses
                limit_number = extract_limit_number(req.message)
